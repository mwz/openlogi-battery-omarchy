...
[documentation](https://github.com/AprilNEA/OpenLogi/blob/master/docs/INSTALL-linux.md#device-access-udev-rules).
...
trailing empty line), 2,048 code units per line, and 24 devices including
offline devices. Device and parent names are limited to 256 code units, kind and
WPID fields to 64, and battery text to 256. Slots must be integers from 0
to 255. Malformed or excessive input produces an error instead of a partial
inventory. Names and errors are rendered as plain text.
...
refresh coalescing, startup failure, and recovery. No connected hardware is
required.
